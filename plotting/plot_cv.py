...
def get_result_path(dataset, alg):
    return f'experiments/results/cv/{dataset}_{alg}.json'
...
def plot_cv_dataset_depth(dataset, depth, test):
    # helper function
    def get_trees(file_path, d, test_loss=False):
        ...
        regs = []
        for tree in results[f'depth_{d}'][f'fold_1']:
            regs.append(tree['regularization'])

        for reg in regs:
            reg_num_leaves = []
            reg_loss = []
            for fold in range(1, 6):
                for tree in results[f'depth_{d}'][f'fold_{fold}']:
                    if reg == tree['regularization']:
                        ...
                        reg_loss.append(np.sqrt(loss))
                        break

            x_iqr = np.quantile(reg_num_leaves, [0.25, 0.5, 0.75])
            y_iqr = np.quantile(reg_loss, [0.25, 0.5, 0.75])
            point = (
                x_iqr[1], y_iqr[1],
    ...
    if test:
        plt.ylabel('Testing Loss (RMSE)')
        plt.title('CV Testing Loss vs Number of Leaves\n ' + dataset + ', max depth: ' + str(depth))
    else:
        plt.ylabel('Training Loss (RMSE)')
        plt.title('CV Training Loss vs Number of Leaves\n ' + dataset + ', max depth: ' + str(depth))

    plt.legend(loc='upper right')
    plt.tight_layout()
    fig_path = Path(f'figures/cv/{dataset}_depth{depth}_{"test" if test else "train"}.png')
